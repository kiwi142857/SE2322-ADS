...
int main1() {
    ...
    single_insert_time = insert_record.get_elapsed_time_micro() * 1000 / base_n_vec; // convert to ns
    printf("average insert time %.1f ns\n", single_insert_time); // print time in ns
    printf("querying\n");
    ...
    single_query_time = query_record.get_elapsed_time_micro() * 1000 / query_n_vec; // convert to ns

    double recall = count_recall(gnd_n_vec, gnd_vec_dim, test_gnd_l, gnd);
    printf("average recall: %.3f, single query time %.1f ns\n", recall, single_query_time); // print time in ns
    return 0;
}
